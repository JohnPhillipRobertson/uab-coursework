...
def tires():
    ...
    tire_names = [name_url(i, False) for i in tires]
    final = zip(tires, tire_names)
    return render_template("tires.html", tires=final)


@app.route("/wheels", methods=['GET', 'POST'])
def wheels():
    wheels = get_urls(wheel_urls, r'\/buy-wheels\/[a-zA-Z0-9\-]*\?')
    wheel_names = [name_url(i, True) for i in wheels]
    final = zip(wheels, wheel_names)
    return render_template("wheels.html", wheels=final)
...
def search():
    ...
    if form.validate_on_submit() == True:
        ...
        global myQuery
        if a and b:
            myQuery = Appointment.query.filter(
                Appointment.firstname == a, Appointment.lastname == b)
        elif a:
